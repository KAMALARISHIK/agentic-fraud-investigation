...
def write_case_to_graph(case_data: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    try:
        ...
        emb = generate_embedding(f"Case {case_id}: {summary_text}")

        # 2. Add InvestigationCase Vertex via MCP tigergraph__add_node
        case_attrs = {
        ...
        # 4. Link Findings (Evidence Claims) via MCP
        for idx, ev in enumerate(case_body.get("evidence", [])):
            fid = f"FINDING-{case_id}-{idx+1}"
            f_attrs = {
